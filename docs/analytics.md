...
Агрегаты строятся для политик `all` и global `latest`, с московской календарной датой и timing buckets `0..30`. После изменения завершённых попыток generation становится stale, API отвечает `409 analytics_refresh_required`, а admin UI предлагает кнопку «Обновить аналитику». Пересчёт выполняется одной D1-транзакцией; candidate flow продолжает работать.
