...
После нового завершённого теста analytics generation помечается устаревшим. Панель получает `409 analytics_refresh_required` и показывает кнопку «Обновить аналитику». Запрос подписан CSRF token и атомарно перестраивает persisted aggregates; на наборе 10 000 / 300 000 локальный пересчёт занимает примерно 12–16 секунд. Candidate flow при этом не останавливается.
...
Baseline 2026-08-28: raw overview `p95 847,9 мс`, raw question-list `p95 3 590 мс`, rebuild `13,33 с`, persisted reads `p95 20,9–220,2 мс`. Поэтому `409 analytics_refresh_required` и явная кнопка пересчёта — эксплуатационный предохранитель, а не ошибка доступности: raw fallback уже на acceptance-наборе нарушает SLA 500 мс.
