...
import { fetchDerivedTrendsReport } from '@/lib/analytics-derived.ts';
...
export async function GET(request: Request) {
  try {
    ...
    return adminJson(await fetchDerivedTrendsReport(
      context.db,
      context.query,
      context.flags.calibration,
    ));
  } catch {
    console.error('admin_analytics_trends_failed');
