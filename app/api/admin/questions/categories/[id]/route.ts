...
import { renameQuestionCategory } from '@/lib/question-bank-workflow.ts';

function categoryId(value: string) {
  const id = Number(value);
  if (!Number.isInteger(id) || id <= 0) throw new QuestionAdminServiceError('invalid_request', 400);
  return id;
}
...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return questionAdminJson(await renameQuestionCategory(
      database(),
      categoryId(id),
    ...
    console.error('admin_question_category_rename_failed');
